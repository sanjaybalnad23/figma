...
export function hexToRbg(color: string): Color {
...
export function penPointsToPathLayer(points: number[][], color: Color): PathLayer {
  ...
  return {
    ...
    opacity: 100,
    points: points
      .filter((point): point is [number, number, number] => point[0] !== undefined && point[1] !== undefined && point[2] !== undefined)
...
export function resizeBounds(bounds: XYWH, corner: Side, point: Point): XYWH {
  ...
  if (corner === Side.Top || (corner & Side.Bottom) !== 0) {
