            ...
            <div className="border-b border-gray-200"></div>
            {
              layer ? (
                <>
                  <div className="flex flex-col gap-1 p-4">
                    ...
                    <div className="flex w-full gap-2">
                      <NumberInput value={layer.x} className='w-1/2' onChange={(value) => updateLayer({ x: value })} icon={<p>X</p>} />
                      <NumberInput value={layer.y} className='w-1/2' onChange={(value) => updateLayer({ y: value })} icon={<p>Y</p>} />
                    </div>
                  </div>
                  {layer.type !== LayerType.Path && <>
                    ...
                    <div className="flex flex-col gap-1 p-4">
                      ...
                      <div className="flex w-full gap-2">
                        <NumberInput value={layer.width} className='w-1/2' onChange={(value) => updateLayer({ width: value })} icon={<p>W</p>} />
                        <NumberInput value={layer.height} className='w-1/2' onChange={(value) => updateLayer({ height: value })} icon={<p>H</p>} />
                      </div>
                    </div>
                  </>
                  }
                  ...
                  <div className="flex flex-col gap-2 p-4">
                    ...
                    <ColorPicker onChange={(color) => { updateLayer({ fill: color }) }} value={rgbToHex(layer.fill)} />
                  </div>
                  ...
                  <div className="flex flex-col gap-2 p-4">
                    ...
                    <ColorPicker onChange={(color) => { updateLayer({ stroke: color }) }} value={rgbToHex(layer.stroke)} />
                  </div>
                  ...
                  {layer.type === LayerType.Text && (
                    <>
                      ...
                      <div className="flex flex-col gap-2 p-4">
                        ...
                        <Dropdown value={layer.fontFamily} onChange={(value) => updateLayer({ fontFamily: value })} options={FONTS} />

                        <div className="flex w-full gap-2">
                          <div className="flex w-full flex-col gap-1">
                            <p className="text-[9px] font-medium text-gray-500">Size</p>
                            <NumberInput icon={<p>W</p>} value={layer.fontSize} onChange={(value) => updateLayer({ fontSize: value })} />
                          </div>
                          <div className="flex w-full flex-col gap-1">
                            <p className="text-[9px] font-medium text-gray-500">Size</p>
                            <Dropdown value={layer.fontWeight.toString()} onChange={(value) => updateLayer({ fontWeight: Number(value) })} options={FONT_WEIGHTS} />
                          </div>
                        </div>
                      </div>
                    </>
                ...
                <div className="flex flex-col gap-2 p-4">
                  ...
                  <ColorPicker onChange={(color) => { setRoomColor(color) }} value={roomColor ? rgbToHex(roomColor) : "#ffffff"} />
                </div>
              )
            }
