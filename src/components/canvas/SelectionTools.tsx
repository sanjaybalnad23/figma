    ...
    return(
        ...
        style={{transform:`translate(calc(${x}px - 50%), calc(${y}px - 100%))`}}
         className="absolute flex min-w[150px] flex-col rounded-xl bg-[#1e1e1e] p-2 shadow-lg">
            <button onClick={bringToFront} className="flex w-full items-center justify-between rounded-md px-1 py-1 text-white hover:bg-blue-500"><span className="text-xs">Bring to front</span><BsArrowDown className="size-4 mr-2"/></button>
            <button onClick={sendToFront} className="flex w-full items-center justify-between rounded-md px-1 py-1 text-white hover:bg-blue-500"><span className="text-xs">Send to back</span><BsArrowUp className="size-4 mr-2"/></button>
        </div>
    )
}